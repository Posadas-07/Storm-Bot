...
if (update.action === "add" && welcomeActivo) {
  for (const participant of update.participants) {
    ...
    const audioUrl = 'https://cdn.russellxz.click/d584a6b7.mp3';
    await sock.sendMessage(update.id, {
      audio: { url: audioUrl },
      mimetype: 'audio/mp4',
      ptt: true
...
if (update.action === "remove" && despedidasActivo) {
  for (const participant of update.participants) {
    ...
    const audioUrl = 'https://cdn.russellxz.click/60f697f4.mp3'; // <- cámbialo si tienes otro audio
    await sock.sendMessage(update.id, {
      audio: { url: audioUrl },
      mimetype: 'audio/mp4',
      ptt: true
...
sock.ev.on("messages.upsert", async (messageUpsert) => {
  try {
    ...
    if (!msg) return;
    // MATRIMONIO: manejar respuestas "sí"/"no" al mensaje de propuesta
if (
    msg.message?.extendedTextMessage?.contextInfo?.quotedMessage &&
    typeof (msg.message.conversation || msg.message?.extendedTextMessage?.text) === "string"
) {
    await matrimonio.onReplyMarriage(msg, { conn: sock });
}
